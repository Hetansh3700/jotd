# This directory is a vault — a proactive notes brain

You are working inside a personal notes vault managed by `vault` (vault-brain).
...
1. **`inbox/` is append-only.** Never edit, rewrite, or delete inbox files. New captures go
   through `vault add "text"` (or the `/capture` command) — nothing else. Permission rules
   deny writes there; do not work around them.
2. **`state/` is derived or CLI-owned.** Never write anything in `state/` directly.
   `processed.log` is written only by `vault mark-processed`; `open-loops.md`, `entities.json`
   are rebuilt by `vault derive`; `pulse-log.md` is written only by the pulse runner and the
   feedback CLI (`vault done|snooze|drop`).
...
Mark a loop finished by flipping `[ ]` to `[x]` (or `vault done <id>`), not by deleting it.
...
- A capture may fan out to several notes; record ALL of them in one
  `vault mark-processed <id> <path,path>` call.
